...
export function renderDocument(data: MndaFormData): string {
  const isExpires = data.mndaTermType === 'expires'
  const isYears = data.confidentialityTermType === 'years'

  const mndaTermLabel = isExpires
    ? `${data.mndaTermYears} year(s) from Effective Date`
    : 'the date of termination'

  const confTermLabel = isYears
...
- [${isExpires ? 'x' : ' '}] Expires ${isExpires ? data.mndaTermYears : '___'} year(s) from Effective Date.
- [${isExpires ? ' ' : 'x'}] Continues until terminated in accordance with the terms of the MNDA.

### Term of Confidentiality

- [${isYears ? 'x' : ' '}] ${isYears ? data.confidentialityTermYears : '___'} year(s) from Effective Date, but in the case of trade secrets until Confidential Information is no longer considered a trade secret under applicable laws.
- [${isYears ? ' ' : 'x'}] In perpetuity.
